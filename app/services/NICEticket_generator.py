...
def generate_tickets_from_activity(activity: Activity, db: Session):
    ...
    owner_id = activity.accompagnato_da or activity.owner_id
    owner_name = activity.owner_name or "N/A"

    # ✅ Gestione speciale: Incarico 24 mesi
    if "ulisse" in activity.title.lower() or "incarico 24 mesi" in activity.title.lower():
        ...
        for task_title in default_tasks:
            task = Task(
                ...
                title=task_title,
                priority="media",
                description=f"{task_title} - {activity.customer_name}",
                owner=owner_name,
                customer_name=activity.customer_name
            )
            db.add(task)
