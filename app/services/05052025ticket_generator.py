...
def generate_tickets_from_activity(activity: Activity, db: Session):
    ...
    if int(activity.sub_type_id) == 63705:
        ...
        # Recupero del nome Owner (Luca Sala) direttamente dai dati
        owner_name = activity.accompagnato_da_nome  # Luca Sala

        # Recupero del nome Account (Stefano Andrello) direttamente dai dati
        account_name = activity.owner_name  # Stefano Andrello
        ...
        parent_ticket = Ticket(
            ...
            gtd_type="Project",
            owner_id=owner_id,
            owner=activity.owner_name,  # Nome dell'owner (Stefano Andrello)
            account=account_name,  # Account: Stefano Andrello
        ...
        # Imposta la due_date del ticket e la salva
    ...
    for process_code in process_codes:
        ...
        parent_ticket = Ticket(
            ...
            gtd_type="Project",
            owner_id=owner_id,
            owner=activity.owner_name,  # Owner: come prima
            status=STATUS_MAP.get(activity.status, 0),
            priority=1,
            description=activity.description,
            customer_name=activity.customer_name,
