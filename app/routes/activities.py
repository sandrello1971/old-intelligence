...
@router.get("/activities/tree", response_model=List[ActivityTreeSchema])
...
def get_activity(activity_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "company_name": activity.customer_name or "-",    # 👈 OK
        "account_name": activity.account_name or "-"      # 👈 OK
    }


@router.patch("/activities/{activity_id}/services")
...
def sync_activity_from_crm(activity_id: int, db: Session = Depends(get_db)):
    """
    Sincronizza una singola attività dal CRM e la aggiorna nel DB locale.
    """
    activity = sync_single_activity(activity_id, session=db)
    if not activity:
        raise HTTPException(status_code=404, detail="Attività non trovata nel CRM o nel DB")
    return {"success": True, "id": activity_id}

@router.put("/activities/{activity_id}/services")
def update_detected_services_put(activity_id: int, payload: dict, db: Session = Depends(get_db)):
    """PUT version per compatibilità frontend"""
