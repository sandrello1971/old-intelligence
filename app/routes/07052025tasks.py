...
def get_task_detail(task_id: int, db: Session = Depends(get_db)):
    ...
    due_date = task.ticket.due_date if task.ticket else None  # Ottieni la due_date dal ticket
    ...
    return {
        ...
        "due_date": due_date,  # Includi la due_date dal ticket
...
def update_task(task_id: int, payload: dict, db: Session = Depends(get_db)):
    ...
    # Aggiorna servizi nel ticket se forniti
...
def confirm_create_opportunities(task_id: int, db: Session = Depends(get_db)):
    ...
    # ⚠️ Prima usa i servizi già salvati, se ci sono
    detected_services = ticket.activity.detected_services

    if not detected_services:
        # Altrimenti ricalcolali dalla descrizione
