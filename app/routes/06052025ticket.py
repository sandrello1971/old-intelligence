...
def get_ticket(ticket_id: int, db: Session = Depends(get_db)):
    ...
    # Account CRM → owner_id
    account = db.query(User).filter(User.id == ticket.owner_id).first()
    account_name = f"{account.name} {account.surname}" if account else None
    ...
    return {
        ...
        "activity": {
            ...
            "detected_services": ticket.activity.detected_services if ticket.activity and ticket.activity.detected_services else []
        },
...
def auto_generate_from_services(ticket_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for service in services:
            ...
            new_ticket_code = f"TCK-{service[:3].upper()}-{activity_id_suffix}-01"
