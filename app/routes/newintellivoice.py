...
def get_services_list(db: Session) -> str:
    """Ottiene lista servizi dinamica dalla tabella sub_types"""
    try:
        # Escludi I24 e TICKET_INT dalla lista servizi selezionabili
        services = db.query(SubType).filter(
            SubType.code.notin_(['I24', 'TICKET_INT'])
        ).all()
        
        services_text = "\n".join([f"- {service.name}" for service in services])
        print(f"[DEBUG] Lista servizi dinamica generata: {len(services)} servizi")
        return services_text
    except Exception as e:
        print(f"[ERROR] Errore recupero servizi dalla DB: {e}")
...
@router.post("/extract")
def extract_entities(payload: TextInput, db: Session = Depends(get_db)):
    print("Testo da analizzare:", payload.text)

    try:
        # ✅ DINAMICO: Ottieni lista servizi dalla DB
        services_list = get_services_list(db)
...
async def create_ulisse_activity(
    ...
    try:
        ...
        actual_company_name = company.nome if company else data.azienda
        
        print(f"[DEBUG] Azienda cercata: '{data.azienda}' → Trovata nel DB: '{actual_company_name}' (ID: {company_id})")

        activity_data = {
            "idCompanion": 120385,
            "subTypeId": 63705,  # I24 - Incarico 24 mesi
        ...
        servizi_text = ", ".join(data.servizi)
        return {
            "id": response,
            "success": True,
            "azienda": data.azienda,
            "azienda_database": actual_company_name,  # ✅ NUOVO: Nome effettivo dal DB
            "servizi": data.servizi,
            "servizi_text": servizi_text,
            "message": f"✅ Attività I24 creata per {actual_company_name} - Servizi: {servizi_text}"
        }
    except HTTPException:
        raise
    except Exception as e:
...
print("ulisse_voice router loaded with dynamic services")
