...
from app.services.crmsdk import get_company_id_by_name  # ✅ corretta
...
def extract_entities(payload: TextInput):
    try:
        ...
        print(">>> Risposta estratta da OpenAI:", data)
...
def create_ulisse_activity(data: ActivityInput, db: Session = Depends(get_db)):
    try:
        ...
        print(f"Servizi: {data.servizi}")
        print(f"Priorita: {data.priority}")

        company_id = get_company_id_by_name(data.azienda, db)
        if not company_id:
            raise HTTPException(status_code=404, detail="Azienda non trovata nel database")
        ...
        print(">>> Payload da inviare al CRM:", activity_data)

        response = create_crm_activity(activity_data)
        print(">>> Risposta dal CRM:", response)

        new_id = response

        print(f">>> Nuova attivita CRM ID: {new_id}")

        sync_single_activity(new_id, db)
        return {"id": new_id}

    except Exception as e:
        print(f">>> ERRORE nella creazione attivita: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
